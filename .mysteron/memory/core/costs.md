...
- **Daily buckets** (`DailyCost`) carry `totalUsd`, `runs` AND `tickets` (distinct tickets that day). The per-day ticket count is tracked via internal `DailyAcc` sets (`bumpDaily`/`sortedDaily`) and lets the UI plot **avg cost per ticket over time** = `totalUsd / tickets` per day, both overall and per-project.
- **HTTP:** `GET /api/costs` → `costStats()`. Web UI is `web/src/Costs.tsx` at route `#/costs` (header `$`-icon link), with stat cards, a dependency-free daily bar chart, an **avg-cost-per-ticket trend chart** (`AvgTicketTrend` — SVG line + least-squares dashed trend line via `trendEnds()`, with a project dropdown defaulting to "all"), and project/ticket tables.
